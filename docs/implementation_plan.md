...
**作業内容:**
- [x] ディレクトリ構造を作成
- [x] pyproject.toml または requirements.txt を作成
- [ ] README.mdに基本的な説明を記載
...
- [x] config.pyモジュールを実装
- [x] 設定ファイルの読み込みロジック
- [x] 必須項目チェック（なければ起動時エラー）
- [x] ランタイム設定ストア（読み込み＋API反映）の実装
- [x] config/ghost.toml.exampleを作成
...
**作業内容:**
- [x] db.pyでSQLAlchemyエンジンとセッション管理を実装
- [x] models.pyで3つのテーブルのORMモデルを定義
- [ ] sqlite-vecの初期化ロジック
- [x] テーブル作成スクリプト（初回起動時）
- [ ] ベクタインデックスの作成処理
...
**作業内容:**
- [x] schemas.pyに全てのリクエスト/レスポンスモデルを定義
- [x] バリデーションルールを設定
...
- [x] 認証の適用
- [x] リクエスト処理の委譲
- [x] LLMエラー時のエラーメッセージ返信（HTTP 200でエラー内容を返す方針を明記）

#### 4.2 /notification エンドポイント（api/notification.py）

**作業内容:**
- [x] api/notification.pyを実装
- [x] /notificationエンドポイントの定義
- [x] 認証の適用

#### 4.3 /meta_request エンドポイント（api/meta_request.py）

**作業内容:**
- [x] api/meta_request.pyを実装
- [x] /meta_requestエンドポイントの定義
- [x] 認証の適用
...
**作業内容:**
- [x] api/capture.pyを実装
- [x] /captureエンドポイントの定義（撮影そのものは別アプリが行い、ここには画像パスや要約テキストが通知される想定）
- [x] 保存除外キーワード設定を参照し、該当する場合は記録スキップを返す
- [x] 認証の適用
...
**作業内容:**
- [x] api/episodes.pyを実装
- [x] /episodesエンドポイントの定義
- [x] 認証の適用
- [x] ページネーション実装
...
**作業内容:**
- [x] ランタイム設定ストアの依存提供関数 `get_config_store` を実装
- [x] 保存除外キーワードを API で更新し、その場で反映
- [x] 介入頻度やキャラクタープロンプトなども更新対象に含める
- [x] 認証の適用とバリデーション

#### 4.7 main.pyへのルーター登録

**作業内容:**
- [x] main.pyに各APIルーターを登録
- [x] 認証依存関係の設定
...
**作業内容:**
- [x] logging設定をconfig.pyまたは独立したlogging_config.pyに実装
- [x] 各モジュールでロガーを設定
- [x] 適切なログレベルでメッセージを記録
- [ ] ログファイルのローテーション設定
...
1. **LLMクライアントとプロンプトの実装**: litellm 呼び出し・実プロンプト組み込み
2. **reflection/memory 強化**: reflection生成・人物更新・エピソード保存フローの実装
3. **sqlite-vec 組み込み**: ベクタインデックス初期化と類似検索API利用
4. **ログ/クリーンアップ整備**: 主要イベントのロギングと生画像72時間削除タスク
5. **テスト追加**: API/メモリのユニットテストとモック整備
