# AI人格の反射/機嫌（persona_affect / persona_mood）仕様
...
## 即時反応（persona_affect trailer）

### 出力形式（LLM → サーバ）

`/api/chat` では、返答本文の直後に次の区切り文字を1行で出力し、その次行に JSON を1つだけ出力します。

- 区切り文字: `<<<COCORO_GHOST_PERSONA_AFFECT_JSON_v1>>>`

サーバ側は区切り以降を **SSEに流さず** 回収し、`units` / `payload_episode` に即時反映します（実装: `cocoro_ghost/memory.py`）。

### 内部JSON（persona_affect trailer）

JSONスキーマは `docs/prompts.md` の「chat（SSE）: 返答末尾の内部JSON（persona_affect trailer）」を参照してください。

## 持続（重要度×時間減衰の集約 → persona_mood）

直近N件の平均では「大事件が短時間で埋もれる」ため、エピソードごとに影響度を定義して積分します。

### 影響度（エピソード i）

`impact_i = persona_affect_intensity_i × salience_i × confidence_i × exp(-Δt / τ_i)`

- `persona_affect_intensity`（0..1）: その瞬間の感情反応の強さ
...
既定（実装: `cocoro_ghost/persona_mood.py`）:
...
### 行動方針ノブ（persona_response_policy）

内部JSONに `persona_response_policy` がある場合、機嫌（mood）の状態に反映します。
...
`label`/`intensity` は「今の機嫌が何っぽいか（状態の要約）」、`persona_response_policy` は「返答生成での振る舞い（行動方針のノブ）」で役割が違うため分けています。
...
2) **LLMの内部JSON（persona_affect trailer）による間接/直接の制御**

`/api/chat` の内部JSONは Episode に保存され、次ターン以降の機嫌計算に取り込まれます。

- 間接: `persona_affect_label/intensity/salience/confidence` によって `anger` が上がれば、結果として `refusal_allowed` が立ちやすくなる
- 直接（上書き候補）: 内部JSONに `persona_response_policy.refusal_allowed` を含めた場合、
